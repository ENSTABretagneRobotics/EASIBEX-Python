...
def i_defaultintfn1arg(X_p, function_p):
    ...
    pz = i_createpointer(nb, n, m, 1, ctypes.c_int)
    pX_p = i_intervalreshapetopointer(X_p, nb, n, m)

    hApiProto = ctypes.CFUNCTYPE(None, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_double), ctypes.c_uint, ctypes.c_uint, ctypes.c_uint)
    hApiParams = (1, "pz", 0), (1, "pX_p", 0), (1, "nb", 0), (1, "n", 0), (1, "m", 0),
    function_call = hApiProto((function_p, hDll), hApiParams)

    function_call(pz, pX_p, ctypes.c_uint(nb), ctypes.c_uint(n), ctypes.c_uint(m))
