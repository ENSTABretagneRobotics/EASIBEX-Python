...
def i_Cdefaultquinarycontractor(Z_p, V_p, W_p, X_p, Y_p, function_p):
    ...
    pV = reshapetopointer(V_p, nb, n, m)
    pW = reshapetopointer(W_p, nb, n, m)
    pX = reshapetopointer(X_p, nb, n, m)
    pY = reshapetopointer(Y_p, nb, n, m)

    hApiProto = ctypes.CFUNCTYPE(ctypes.c_int, ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.c_uint, ctypes.c_uint, ctypes.c_uint)
    hApiParams = (1, "pZ", 0), (1, "pV", 0), (1, "pW", 0), (1, "pX", 0), (1, "pY", 0), (1, "nb", 0), (1, "n", 0), (1, "m", 0),
    function_call = hApiProto((function_p, hDll), hApiParams)

    function_call(pZ, pV, pW, pX, pY, ctypes.c_uint(nb), ctypes.c_uint(n), ctypes.c_uint(m))
    
    # Conversions to human-readable format.
    Z = reshapefrompointer(pZ, nb, n, m)
    V = reshapefrompointer(pV, nb, n, m)
