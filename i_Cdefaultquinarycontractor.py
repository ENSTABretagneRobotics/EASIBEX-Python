...
def i_Cdefaultquinarycontractor(Z_p, V_p, W_p, X_p, Y_p, function_p):
    ...
    pV_p = i_intervalreshapetopointer(V_p, nb, n, m)
    ...
    hApiProto = ctypes.CFUNCTYPE(None, ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.c_uint, ctypes.c_uint, ctypes.c_uint)
    hApiParams = (1, "pZ_p", 0), (1, "pV_p", 0), (1, "pW_p", 0), (1, "pX_p", 0), (1, "pY_p", 0), (1, "nb", 0), (1, "n", 0), (1, "m", 0),
    function_call = hApiProto((function_p, hDll), hApiParams)

    function_call(pZ_p, pV_p, pW_p, pX_p, pY_p, ctypes.c_uint(nb), ctypes.c_uint(n), ctypes.c_uint(m))
    
    # Conversions to human-readable format.
    Z = i_intervalreshapefrompointer(pZ_p, nb, n, m)
    V = i_intervalreshapefrompointer(pV_p, nb, n, m)
