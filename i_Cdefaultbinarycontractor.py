...
def i_Cdefaultbinarycontractor(Y_p, X_p, function_p):
    ...
    hApiProto = ctypes.CFUNCTYPE(ctypes.c_int, ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.c_uint, ctypes.c_uint, ctypes.c_uint)
    hApiParams = (1, "pY", 0), (1, "pX", 0), (1, "nb", 0), (1, "n", 0), (1, "m", 0),
    function_call = hApiProto((function_p, hDll), hApiParams)

    function_call(pY, pX, ctypes.c_uint(nb), ctypes.c_uint(n), ctypes.c_uint(m))
